...
apiVersion: v1
kind: ConfigMap
metadata: { name: devops-app-config }
data:
  # Simple key/values — used as env vars
  APP_NAME: "DevOps Demo App"
  ENVIRONMENT: "staging"
  # Larger config can be a multi-line block — used as a file
  motd.txt: |
    Welcome to DevOps Fundamentals.
    Build #${CI_BUILD_NUMBER} (placeholder).
```
...
spec:
  ...
  template:
    metadata:
      ...
      annotations:
        # Trick to force a rollout when ConfigMap changes:
        # update this hash whenever you change the ConfigMap (or use kustomize).
        config-hash: "v1"
    spec:
      containers:
        - name: app
          ...
          envFrom:
            - configMapRef: { name: devops-app-config }   # all CM keys → env
          volumeMounts:
            - name: motd
              mountPath: /etc/motd                         # mount one file
              subPath: motd.txt
          readinessProbe:
