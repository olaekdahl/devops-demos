...
- A local shell with `bash`, `python3`, `git`. Nothing else.
...
- Walk through one tiny end-to-end "DevOps loop" to make it concrete.
...
A whiteboard / slide-driven demo plus a tiny end-to-end loop in a local shell:
...
- Whiteboard your current employer's "loop" and identify the
  longest-feedback-loop step. That step is the candidate for automation.
- Introduce DORA metrics and estimate them for the demo loop.
